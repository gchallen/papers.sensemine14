...
collected data for 21 days. \XXXnote{stevko: revise the numbers}
...
were collected over 2 days from 17 users.}}
\end{threeparttable}
\end{table*}
...
order to reduce the battery consumption of our tool.\XXXnote{stevko:
...
the tool. \XXXnote{stevko: say something about why we only use 88 devices?}
...
%Though we only have one instance, the implication of
%this for \PhoneLab{} users is that the description they provide needs to explain
%clearly why their experiment requests certain sensitive permissions.
...
%snapshots use \texttt{PhoneLabSystemAnalysis-Snapshot} as the tag, the label
...
scripts. In the end, we have collected \XXXnote{stevko: how much did we
generate?} over 21 days from \XXXnote{88 devices?}.
